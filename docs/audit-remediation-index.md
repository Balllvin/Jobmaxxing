...
Do not treat the proposed fixes as instructions to implement blindly. Each thread must read the named files, verify the failure, challenge the proposed approach, then ship the simplest fix that preserves the product goal: help [Candidate] get a job with truthful, high-signal, user-approved hiring work.
